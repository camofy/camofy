...
（`cf-mitigated: challenge`、`Just a moment...`、`_cf_chl_opt`），挑战页面需要执行
JavaScript，纯 HTTP 客户端无法通过；此时任何请求都会以 403 结束。

2026-09-23 在同一台生产机上实测（每次只换一个变量）：

| 出口 | 客户端指纹 | 结果 |
| --- | --- | --- |
| 服务器自身 IP（154.36.168.183，US/NetLab） | plain curl（非浏览器 TLS） | 403 挑战 |
| 服务器自身 IP | `curl_cffi` 模拟 Chrome / Safari / Firefox | **同样 403 挑战** |
| 携趣短效地址（辽宁辽阳联通 `113.231.30.252:3828`，`cdn-cgi/trace` 确认出口一致） | plain curl | 403 挑战；重试时出现连接被重置 |
| 携趣短效地址（同一出口） | `curl_cffi` 模拟 Chrome | **同样 403 挑战** |
| WestData 订阅节点（`🇨🇳 Taiwan | 08` 等，经本机 mihomo 逐个探测） | .NET / reqwest / mihomo | 200，直接拿到登录表单 |

结论：**这是按来源 IP 下发的托管挑战，不是 TLS/UA 指纹问题**；伪造 JA3/JA4 指纹
（`curl_cffi`、`rquest` 一类）在这些出口上无效，不要往这个方向修。被挑战的判据不是
"境内还是境外"：携趣整条产品线都是公开代理 IP，属于风控标记段；而机场自己的节点
（以及海外干净出口）不被挑战。
可行做法只有两条：

1. 给面板登录换一个不被挑战的出口（实测机场自己的节点与海外干净出口都可行，且订阅拉取
   在这些出口上同样可用，因此可以整条出口替换，或另配一条仅供面板登录使用的出口策略）；
   注意携趣这类公开代理池属于风控标记段，换多少个 IP 都会被挑战；
2. 让供应商提供不走 Cloudflare 的面板入口。

云端不引入浏览器：解 JS 挑战需要真实浏览器运行时，与"镜像内无浏览器、最小体积"
的既有约束冲突。定位这类问题时，服务日志只应记录"收到挑战/拦截页"这一事实与状态码，
不记录页面正文，也不记录 Cookie。
