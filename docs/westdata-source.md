...
| 出口 | 客户端指纹 | 结果 |
| --- | --- | --- |
| 服务器自身 IP（154.36.168.183） | plain curl（非浏览器 TLS） | 403 挑战 |
| 服务器自身 IP | `curl_cffi` 模拟 Chrome / Safari / Firefox | **同样 403 挑战** |
| 携趣短效地址（江苏电信，刚提取） | plain curl | 403 挑战，`cf-mitigated: challenge` |
| 携趣短效地址 | `curl_cffi` 模拟 Chrome | **同样 403 挑战** |
| 台湾节点（103.156.242.224） | .NET / reqwest（非浏览器 TLS） | 200，直接拿到登录表单 |

结论：**这是按来源 IP／地区／ASN 下发的托管挑战，不是 TLS/UA 指纹问题**；
伪造 JA3/JA4 指纹（`curl_cffi`、`rquest` 一类）在这些出口上无效，不要往这个方向修。
可行做法只有两条：

1. 给面板登录换一个不被挑战的出口（实测海外/台湾节点可行）。订阅拉取在该出口上
