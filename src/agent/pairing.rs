...
async fn status(State(w): State<Web>, h: HeaderMap) -> Json<Value> {
    ...
    Json(
        json!({"bound":inner.bound,"cloud_url":inner.cloud,"identity_name":runtime["identity_name"].as_str().unwrap_or(&inner.identity),"runtime":runtime,"phase":if own{inner.phase.as_str()}else{"idle"}}),
    )
}
#[derive(Deserialize)]
struct Action {
